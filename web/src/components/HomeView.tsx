...
export default function HomeView({
  ...
  /** True while the startup effect is scrolling to the sell section. Tells the
   *  scroll-blend effect's mount-time compute() not to clobber the route-resolved
   *  sell view (on mount the page is at scroll-top, which would flip back to buy). */
  const settlingToSellRef = useRef(false);

  // Resolve startup target only after claim blocking status is known.
  // Drives the initial view from the URL (`/buy`, `/sell`) via `initialView`,
  // falling back to the first-sell-visit default on `/`. The claim-blocking
  // gate wins regardless (resolveInitialView returns "buy"); when a pending
  // claim later clears, this effect re-runs (claimBlockingActive is a dep)
  // and the route's view applies.
  useEffect(() => {
    ...
    const { view, source } = resolveInitialView({
    ...
    if (view === "buy") {
      // Claim gate, explicit /buy, or returning-visitor default: stay at the
      // auction/top. The scroll-blend effect handles the claim force-to-buy.
      setInitialViewResolved(true);
      return;
    }

    // view === "sell" — explicit /sell route or a genuine first-sell-visit.
    // Reuse the proven settle-under-boot-overlay path; only a real first visit
    // consumes the localStorage flag (a /sell route visit must not change
    // `/`'s future default).
    viewHintScrollRef.current = "sell";
    setActiveView("sell");
    setScrollBlend(1);
    settlingToSellRef.current = true;
    let cancelled = false;
    let attempts = 0;
    ...
    const settleAtSell = () => {
      if (cancelled) return;
      const finish = () => {
        settlingToSellRef.current = false;
        if (source === "firstVisit") {
  ...
  }, [claimBlockingResolved, claimBlockingActive, initialView]);
  ...
  useEffect(() => {
    ...
    const compute = () => {
      pending = false;
      // Don't clobber the route-resolved sell view while the startup effect is
      // mid-settle: on mount the page is at scroll-top (t=0), which would flip
      // the hysteresis back to "buy" before the scroll lands.
      if (settlingToSellRef.current) return;
